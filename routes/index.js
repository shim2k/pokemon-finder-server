...
router.get('/shim', function(req, res, next) {
  ...
  exec('cd PokemonGo-Map/ && python example.py -a ptc -u ' + credentials.user + ' -p ' + credentials.pass + ' -l "' +  req.query['q'] + '" -st 10 -H "0.0.0.0" -i "Rattata,Paras,Zubat,Growlithe,Ekans,Pidgey"', function(error, stdout, stderr) {
    output(error, stdout, stderr);
  })
  setTimeout(function() {
    res.redirect('/map')
